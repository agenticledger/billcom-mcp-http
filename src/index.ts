#!/usr/bin/env node
/**
 * Bill.com MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Bill.com secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Bill.com
 ...
 *      `Authorization: Bearer <billcom-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Bill.com key, construct the client.
...
const SLUG = 'billcom';
const NAME = 'Bill.com MCP Server';
const VERSION = '2.0.0';

/** Construct a BillcomClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): BillcomClient {
  const o = JSON.parse(credential); return new BillcomClient(o.devKey, o.username, o.password, o.orgId, o.environment ?? process.env.BILLCOM_ENV ?? "sandbox");
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'Bill.com Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Bill.com API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Bill.com key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <billcom-api-key>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: BillcomClient }
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Bill.com isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Bill.com is not connected for this caller yet. Connect it once via the link below (paste your Bill.com API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Bill.com is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-billcom-api-key>' },
      });
      return;
    }
