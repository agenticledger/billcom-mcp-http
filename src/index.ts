#!/usr/bin/env node
/**
 * Bill.com MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their Bill.com credentials as a pipe-delimited Bearer token:
 *   Authorization: Bearer devKey|username|password|orgId|environment
 *
 * The server extracts credentials, creates a per-session BillcomClient.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Bill.com AP, AR, vendors, bills, invoices, payments, and approvals through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Bill.com credentials as a pipe-delimited Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <devKey>|<username>|<password>|<orgId>|<environment>',
      format: 'devKey|username|password|orgId|environment (environment is optional, defaults to sandbox)',
      howToGetKey:
        'Get your Developer Key from the Bill.com Developer Portal at https://developer.bill.com/',
    },
    configTemplate: {
      mcpServers: {
        billcom: {
    ...
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/billcom/',
    },
  });
});
  ...
  const configTemplate = JSON.stringify(
    {
      mcpServers: {
        billcom: {
          url: `${SERVER_BASE_URL}/mcp`,
          headers: {
            Authorization: 'Bearer YOUR_DEV_KEY|YOUR_USERNAME|YOUR_PASSWORD|YOUR_ORG_ID|sandbox',
          },
        },
      },
    },
...
<head>
  ...
  <title>Bill.com MCP Server — AgenticLedger</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    input[type="text"],input[type="password"]{width:100%;padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;transition:border-color .15s;margin-bottom:0.5rem}
    input[type="text"]:focus,input[type="password"]:focus{border-color:#2563EB;box-shadow:0 0 0 3px rgba(37,99,235,.12)}
    select{width:100%;padding:0.6rem 0.75rem;font-size:0.95rem;font-family:inherit;border:1px solid #cbd5e1;border-radius:8px;outline:none;margin-bottom:0.5rem}
...
<body>
  ...
  <div class="container">
    <h1>Bill.com MCP Server</h1>
    <p class="subtitle">Access Bill.com AP, AR, vendors, bills, invoices, payments, and approvals through ${tools.length} MCP tools.</p>

    <div class="card">
      <h2>How to get your Bill.com credentials</h2>
      <ol class="steps">
        <li>Get a <strong>Developer Key</strong> from the <a href="https://developer.bill.com/" target="_blank" rel="noopener">Bill.com Developer Portal</a>.</li>
        <li>Use your Bill.com <strong>username</strong> (email) and <strong>password</strong>.</li>
        <li>Find your <strong>Organization ID</strong> in Bill.com settings.</li>
        <li>Choose <strong>sandbox</strong> or <strong>production</strong> environment.</li>
        <li>Enter values below to generate your config.</li>
      </ol>
    </div>
    ...
    <div class="card">
      ...
      <input type="text" id="devKey" placeholder="Your Bill.com Developer Key" autocomplete="off" spellcheck="false" />
      <label for="username">Username (email)</label>
      <input type="text" id="username" placeholder="user@company.com" autocomplete="off" spellcheck="false" />
      <label for="password">Password</label>
      <input type="password" id="password" placeholder="Your Bill.com password" autocomplete="off" />
      <label for="orgId">Organization ID</label>
      <input type="text" id="orgId" placeholder="Your Bill.com Org ID" autocomplete="off" spellcheck="false" />
      <label for="env">Environment</label>
      <select id="env"><option value="sandbox">Sandbox</option><option value="production">Production</option></select>
  ...
  <script>
    ...
    const inputs = ['devKey','username','password','orgId','env'];

    function buildConfig() {
      const dk = document.getElementById('devKey').value.trim() || 'YOUR_DEV_KEY';
      const un = document.getElementById('username').value.trim() || 'YOUR_USERNAME';
      const pw = document.getElementById('password').value.trim() || 'YOUR_PASSWORD';
      const oi = document.getElementById('orgId').value.trim() || 'YOUR_ORG_ID';
      const en = document.getElementById('env').value;
      const token = dk + '|' + un + '|' + pw + '|' + oi + '|' + en;
    ...
    inputs.forEach(function(id) {
      document.getElementById(id).addEventListener('input', function () {
        document.getElementById('configBlock').textContent = buildConfig();
      });
    });
...
function createMCPServer(client: BillcomClient): Server {
  const server = new Server(
    { name: 'billcom-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  // New session — requires Bearer token with pipe-delimited credentials
  const creds = parseBearerToken(req);
  if (!creds) {
    res.status(401).json({
      error: 'Missing or malformed Authorization header. Use: Bearer <devKey>|<username>|<password>|<orgId>|<environment>',
    });
    return;
  }
...
<head>
  ...
  <title>Authorize \u2014 Bill.com MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Bill.com MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Bill.com MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Bill.com MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Bill.com credentials)`);
});
// OAuth upgrade 2026-03-19T00:52:10Z
